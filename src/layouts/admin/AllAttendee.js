...
function AllAttendee(props) {
  ...
  useEffect(() => {
    const filtered = attendees.filter((attendee) => {
      ...
      const firstnameMatch = attendee.first_name?.toLowerCase().includes(firstNameFilter.toLowerCase()) ?? false;
const emailMatch = attendee.email_id?.toLowerCase().includes(emailIDFilter.toLowerCase()) ?? false;
const companyMatch = attendee.company_name?.toLowerCase().includes(companyFilter.toLowerCase()) ?? false;
const roleMatch = roleFilter === "All" || attendee.status?.toLowerCase() === roleFilter.toLowerCase();
